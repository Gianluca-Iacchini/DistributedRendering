...
ComPtr<ID3D12Resource> CommandList::CopyBuffer( size_t bufferSize, const void* bufferData, D3D12_RESOURCE_FLAGS flags )
{
    ...
    else
    {
        ...
        auto buff = CD3DX12_RESOURCE_DESC::Buffer(bufferSize, flags);

        ThrowIfFailed( d3d12Device->CreateCommittedResource(
            &props, D3D12_HEAP_FLAG_NONE,
            &buff, D3D12_RESOURCE_STATE_COMMON, nullptr,
        ...
        if ( bufferData != nullptr )
        {
            ...
            auto buff = CD3DX12_RESOURCE_DESC::Buffer(bufferSize);

            // Create an upload resource to use as an intermediate buffer to copy the buffer resource
            ComPtr<ID3D12Resource> uploadResource;
            ThrowIfFailed( d3d12Device->CreateCommittedResource(
                &props, D3D12_HEAP_FLAG_NONE,
                &buff, D3D12_RESOURCE_STATE_GENERIC_READ, nullptr,
...
std::shared_ptr<Texture> CommandList::LoadTextureFromFile( const std::wstring& fileName, bool sRGB )
{
    ...
    else
    {
        ...
        ThrowIfFailed( d3d12Device->CreateCommittedResource(
            &props, D3D12_HEAP_FLAG_NONE, &textureDesc,
                                 ...
                                 const std::shared_ptr<Texture>& panoTexture )
{
    ...
    if ( ( cubemapDesc.Flags & D3D12_RESOURCE_FLAG_ALLOW_UNORDERED_ACCESS ) == 0 )
    {
        ...
        ThrowIfFailed( d3d12Device->CreateCommittedResource(
            &props, D3D12_HEAP_FLAG_NONE, &stagingDesc,
                                          ...
                                          uint32_t numSubresources, D3D12_SUBRESOURCE_DATA* subresourceData )
{
    ...
    if ( destinationResource )
    {
        ...
        auto props = CD3DX12_HEAP_PROPERTIES(D3D12_HEAP_TYPE_UPLOAD);
        auto buff = CD3DX12_RESOURCE_DESC::Buffer(requiredSize);
