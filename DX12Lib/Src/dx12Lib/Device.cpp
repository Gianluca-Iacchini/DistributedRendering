...
public:
    ...
    MakeStructuredBuffer( Device& device, Microsoft::WRL::ComPtr<ID3D12Resource> resource, size_t numElements, size_t elementSize )
...
class MakeInputBuffer : public InputBuffer
{
public:
    MakeInputBuffer(Device& device, D3D12_RESOURCE_DESC& resDesc)
        : InputBuffer(device, resDesc)
    {}

    MakeInputBuffer(Device& device, Microsoft::WRL::ComPtr<ID3D12Resource> resource)
        : InputBuffer(device, resource)
    {}

    virtual ~MakeInputBuffer() {}
};
...
std::shared_ptr<StructuredBuffer> Device::CreateStructuredBuffer(Microsoft::WRL::ComPtr<ID3D12Resource> resource, size_t numElements,
                                                                  size_t elementSize )
{
    std::shared_ptr<StructuredBuffer> structuredBuffer =
...
std::shared_ptr<InputBuffer> Device::CreateInputBuffer(int width, int height, DXGI_FORMAT format)
{
    ...
    std::shared_ptr<InputBuffer> inputBuffer = std::make_shared<MakeInputBuffer>(*this, resourceDesc);
